...
public class EventDebouncerTest {
    ...
    public void should_deliver_n_events_in_order_even_if_queue_full() throws InterruptedException {
        ...
        callback.awaitInvocations(50);
        assertThat(callback.getEvents()).isEqualTo(events);
        assertThat(callback.getInvocations()).isEqualTo(50);
    }
    ...
    @Test(groups = "unit")
    public void should_wait_until_ready_if_queue_full() throws InterruptedException {
        ...
        // this thread may block while attempting to insert 26th element
        new Thread(){
            @Override
            public void run() {
                for (int i = 0; i < 50; i++) {
        ...
        new Thread(){
            @Override
            public void run() {
                // this will release the waiting thread
                debouncer.start();
            }
        }.start();
        callback.awaitEvents(50);
        assertThat(callback.getEvents()).isEqualTo(events);
        assertThat(callback.getInvocations()).isEqualTo(2);
    }
